...
class TestCli:
    ...
    def f_get_request(self, mocker) -> Mock:
        return mocker.patch(self.path_util + ".helpers.CLIContext.api_get", return_value=self.response)

    @pytest.fixture(autouse=True)
    def f_post_request(self, mocker) -> Mock:
        return mocker.patch(self.path_util + ".helpers.CLIContext.api_post", return_value=self.response)

    @pytest.fixture(autouse=True)
    def f_delete_request(self, mocker) -> Mock:
        return mocker.patch(self.path_util + ".helpers.CLIContext.api_delete", return_value=self.response)
    ...
    @pytest.mark.parametrize("p_mock_open", [mock_open(), Mock(side_effect=IOError)])
    def test_generate_docs(self, mocker, p_mock_open):
        mocker.patch(self.path_cli + ".helpers.clean_up_documentation")
    ...
    def test_runs_stop(self):
        result = self.runner.invoke(cli, ["runs", "stop", "1"])
    ...
    def test_plan_executions_resume(self):
    ...
    def test_plan_executions_stop(self):
        result = self.runner.invoke(cli, ["plan-executions", "stop", "1"])
    ...
    def test_stage_executions_stop(self):
        result = self.runner.invoke(cli, ["stage-executions", "stop", "1"])
    ...
    def test_step_executions_stop(self):
        result = self.runner.invoke(cli, ["step-executions", "stop", "1"])
    ...
    def test_logs_list(self):
        result = self.runner.invoke(cli, ["logs", "list", "-f", "name", "value"])
