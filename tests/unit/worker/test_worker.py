...
#     def test__stop_task(self, mock_pop_task):
#         mock_task = Mock()
#         mock_pipe = Mock()
#         mock_pop_task.return_value = mock_task
#
#         self.worker_obj._stop_task({co.RESULT_PIPE: mock_pipe, co.CORRELATION_ID: "1"})
#
#         mock_pipe.send.assert_called_once_with({co.RESULT: co.CODE_OK})
#
#     @patch("cryton.worker.consumer.Consumer.pop_task")
#     def test__stop_task_error(self, mock_pop_task):
#         mock_task = Mock()
#         mock_task.stop.side_effect = RuntimeError
#         mock_pop_task.return_value = mock_task
#         mock_pipe = Mock()
#         self.worker_obj._stop_task({co.RESULT_PIPE: mock_pipe, co.CORRELATION_ID: "1"})
...
#
#     @patch("cryton.worker.consumer.Consumer.pop_task")
#     def test__stop_task_not_found(self, mock_pop_task):
