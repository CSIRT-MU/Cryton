...
class Stage(Instance):
    def __init__(self, model_id: int):
        ...
        self._logger = logger.logger.bind(stage_id=model_id, name=self.model.name)
...
class StageExecution(Execution):
    def __init__(self, model_id: int):
        ...
        self._logger = logger.logger.bind(stage_execution_id=model_id, name=self.model.stage.name)
    ...
    def state(self, value: str):
        with transaction.atomic():
            ...
            if st.StageStateMachine.validate_transition(self.state, value):
                self._logger.debug("stage execution changed state", state_from=self.state, state_to=value)
                model = self.model
                model.state = value
                model.save()
    ...
    def start(self):
        self._logger.debug("stage execution starting trigger")
        ...
        try:
            ...
            self._logger.error(f"Unable to start trigger", error=str(ex))
            self.state = st.ERROR
            return
        ...
        self._logger.info("stage execution trigger started")

    def stop(self):
        self._logger.debug("stage execution stopping")
        ...
        self._logger.info("stage execution stopped")
    ...
    def execute(self) -> None:
        ...
        :return: None
        """
        self._logger.debug("stage execution executing")
    ...
    def pause(self):
        ...
        :return: None
        """
        self._logger.debug("stage execution pausing")
        ...
        if self.state in [st.AWAITING]:
            try:
                ...
                self._logger.warning("stage execution cannot be paused", exception=str(ex))
                return
            self.pause_time = timezone.now()
    ...
    def resume(self):
        self._logger.debug("stage execution resuming")
    ...
    def _execute_subjects_to_dependency(self) -> None:
        ...
        :return: None
        """
        self._logger.debug("stage execution running dependency subjects")
    ...
    def re_execute(self, immediately: bool = False) -> None:
        ...
        :return: None
        """
        self._logger.debug("stage execution re-executing")
        ...
        if immediately:
            ...
            self.trigger.start()
        self._logger.info("stage execution re-executed")
    ...
    def _reset_execution_data(self) -> None:
        ...
        :return: None
        """
        self._logger.debug("stage execution resetting")
        st.StageStateMachine.validate_state(self.state, st.STAGE_FINAL_STATES)
        ...
        self._logger.debug("stage execution reset")
    ...
    def finish(self) -> None:
        ...
        self._logger.info("stage execution finished")
